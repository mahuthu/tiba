...
class TriageSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        # Convert empty strings or None to 0 for numeric fields
        numeric_fields = [
            'temperature', 'height', 'weight',
            'pulse', 'systolic', 'diastolic', 'bmi'
        ]
    ...
    def get_patient_name(self, obj):
        # Get patient name through AttendanceProcess
        try:
            attendance_process = obj.attendanceprocess_set.first()
            if attendance_process:
                patient = attendance_process.patient
                return f"{patient.first_name} {patient.second_name}"
        except Exception:
            return None
        return None
    ...
    def to_representation(self, instance):
        data = super().to_representation(instance)
        # Add any additional patient info you need through AttendanceProcess
        try:
            attendance_process = instance.attendanceprocess_set.first()
            if attendance_process:
                data['patient_id'] = attendance_process.patient.id
        except Exception:
...
class AttendanceProcessSerializer(serializers.ModelSerializer):
    ...
    #     triage_data = validated_data.pop('triage', None)
        
    #     # Update triage if provided
    #     if triage_data and instance.triage:
    #         for attr, value in triage_data.items():
    #             setattr(instance.triage, attr, value)
    #         instance.triage.save()
