...
export const getAllProcesses = () => async (dispatch) => {
  try {
    ...
    // Ensure the track property exists and is properly formatted
    const processesWithTrack = response.map(process => ({
      ...process,
      track: process.track?.toLowerCase() || "doctor" // provide default value if track is missing
    }));
    
    console.log("Processed response:", processesWithTrack);
    dispatch(setProcesses(processesWithTrack));
  } catch (error) {
    console.error("ATTENDANCE_PROCESSES_ERROR:", error);
    dispatch(setProcesses([])); // Set empty array on error
  }
};
