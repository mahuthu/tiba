...
const TRACK_GROUPS = {
  ...
  LAB: ['awaiting result', 'added result', 'lab']
};
...
const DoctorPatientDataGrid = () => {
  ...
  const doctorsSchedules = processes.filter((process) => {
    const currentTrack = process.track?.toLowerCase();
    
    if (processTrack === 'doctor') {
      return TRACK_GROUPS.DOCTOR.includes(currentTrack);
    } else if (processTrack === 'lab') {
      return TRACK_GROUPS.LAB.includes(currentTrack);
    }
    return false;
  });
  ...
  useEffect(() => {
    ...
    console.log("Filtered doctorsSchedules:", doctorsSchedules);
    console.log("Current processTrack:", processTrack);
  }, [processes, doctorsSchedules, processTrack]);
  ...
  return (
    <section>
      <div className="capitalize flex gap-4 py-4">
        <p 
          className={`cursor-pointer text-primary border-b px-2 ${processTrack === "doctor" ? "border-primary" : "border-transparent"}`} 
          onClick={() => setProcessTrack("doctor")}
        >
          Needs Doctor&apos;s Attention ({doctorsSchedules.length})
        </p>
        <p 
          className={`cursor-pointer text-primary border-b px-2 ${processTrack === "lab" ? "border-primary" : "border-transparent"}`} 
          onClick={() => setProcessTrack("lab")}
        >
          Lab Results ({doctorsSchedules.length})
        </p>
      </div>
      <div className="text-xs text-gray-500 mb-4">
        <p>Total Processes: {processes.length}</p>
        <p>Filtered Processes: {doctorsSchedules.length}</p>
      </div>
      <DataGrid
