...
import DischargePatientModal from '../reception-interface/discharge-patient-modal';
import AssignNurseModal from '../reception-interface/assign-nurse-modal';
...
const getActions = () => {
    let actions = [
        {
            action: "nurse",  // Add this new action
            label: "Send to Nurse",
        ...
        },
        {
            ...
            icon: <MdOutlinePersonRemoveAlt1 className="text-card text-xl mx-2" />,
        },
    ];
    return actions;
};
...
const AppointmentCard = ({ processes }) => {
    ...
    const handleAppointmentsDialog = () => {
        if (!showAppointments) {
    ...
    const onMenuClick = async (menu, data) => {
        ...
        } else if  (menu.action === "nurse") {  // Add this condition
            setSelectedData({
                ...data,
                patient_name: linkedPatient ? `${linkedPatient.first_name} ${linkedPatient.second_name}` : ""
            });
            setAssignNurseOpen(true);
    ...
    const handleSendToPharmacy = async () => {
        try {
            ...
            console.log("Error sending to pharmacy", error);
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <>
            <div className="flex justify-between items-center mb-4">
                ...
                <IconButton 
                    onClick={handleAppointmentsDialog}
            ...
            <Dialog
                ...
                onClose={handleAppointmentsDialog}
                maxWidth="lg"
                fullWidth
            >
                <DialogTitle className="flex justify-between items-center">
                    <span className="text-xl font-semibold">Patient Appointments</span>
                    <IconButton onClick={handleAppointmentsDialog}>
                ...
                <DialogContent>
                    <PatientAppointmentDataGrid patientAppointments={patientAppointments} />
                </DialogContent>
            </Dialog>

            {/* Other Modals */}
            {assignOpen && <AssignDoctorModal {...{ assignOpen, setAssignOpen, selectedData }} />}
            {labOpen && <DirectToTheLabModal {...{ labOpen, setLabOpen, selectedData }} />}
            {dischargeOpen && (
                <DischargePatientModal 
                    selectedRecords={[selectedData]} 
                    open={dischargeOpen}
                    setOpen={setDischargeOpen}
                />
            )}
            {assignNurseOpen && (
            <AssignNurseModal 
                assignNurseOpen={assignNurseOpen}
                setAssignNurseOpen={setAssignNurseOpen}
